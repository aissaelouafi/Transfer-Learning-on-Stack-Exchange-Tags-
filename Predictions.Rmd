...
```{r}
test_corpus <- generateCorpus(test$content)
test_corpus <- DocumentTermMatrix(test_corpus)
test_corpus
```
...
ldaOut <-LDA(test_corpus,50, method="Gibbs",control=control)
```
...
travel_tags <- travel_tags[,c("id","tags")]
travel_tags
sample_submission$tags <- travel_tags$tags
write.csv(sample_submission, file = "submission.csv",row.names = FALSE)
```
...
plot_ly(x = nrow(eig.dataframe):1, y = eig.dataframe$cumvariance,type="bar",name="PCs importance") %>% layout(yaxis = list(title = 'Cumulative variance'),xaxis = list(title = "Principal components")) %>% add_trace(x=nrow(eig.dataframe):1,y=eig.dataframe$cumvariance,mode = "lines",name="PC importance")
```
